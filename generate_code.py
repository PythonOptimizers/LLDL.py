#!/usr/bin/env python
################################################################################
# This script generates all templated code for LLDL.py
# It this the single one script to use before Cythonizing the LLDL.py
# library.
...
INDEX_TYPES = ['INT64']  # ['INT32','INT64']

COMPLEX_ELEMENT_TYPES = ['COMPLEX64', 'COMPLEX128']
REAL_ELEMENT_TYPES = ['FLOAT32', 'FLOAT64']
ELEMENT_TYPES = ['FLOAT64']  # COMPLEX_ELEMENT_TYPES + REAL_ELEMENT_TYPES
...
def create_logger(lldl_config):
    logger_name = lldl_config.get('CODE_GENERATION', 'log_name')
    if logger_name == '':
        logger_name = 'lldl_generator'

    logger = logging.getLogger(logger_name)

    # levels
    log_level = LOG_LEVELS[lldl_config.get('CODE_GENERATION',
                                           'log_level')]
    console_log_level = LOG_LEVELS[lldl_config.get('CODE_GENERATION',
                                                   'console_log_level')]
    file_log_level = LOG_LEVELS[lldl_config.get('CODE_GENERATION',
                                                'file_log_level')]
...
if __name__ == "__main__":
    ...
    # create logger
    logger = create_logger(lldl_config)
    ...
    cygenja_engine.register_action('lldl', '*.*',
                                   single_generation)
    cygenja_engine.register_action('lldl/src', '*.*',
