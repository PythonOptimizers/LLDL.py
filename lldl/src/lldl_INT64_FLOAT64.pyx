...
def lldl_INT64_FLOAT64(unsigned int n, unsigned int m,
                          adiag,
                          colptrT,
                          rowindT,
                          valuesT,
...
cpdef dense_lsolve_INT64_FLOAT64(n,
    ...
    for j in xrange(n):
        for p in xrange(Li[j], Li[j + 1]):
            x[Lr[p]] -= Lv[p] * x[j]
    return

cpdef dense_ltsolve_INT64_FLOAT64(n,
                                   cnp.ndarray[cnp.int64_t, ndim=1] Li,
                                   cnp.ndarray[cnp.int64_t, ndim=1] Lr,
                                   cnp.ndarray[cnp.float64_t, ndim=1] Lv,
                                   cnp.ndarray[cnp.float64_t, ndim=1] x):
